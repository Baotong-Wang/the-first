	...
	<body>
		...
		<!-- 我们需要用到当前的jQuery对象，进行数据的访问，通过AJAX -->
	 	<script type="text/javascript">
	 		...
	 		//通过ajax去请求服务器，获取对应的部门信息数据
	 		$.post("/Department/SelectServlet",function(data){
	 			//第一步：将字符串封装成集合对象
	 			var json = eval("(" + data + ")");
	 			//循环遍历的里面
	 			for(var i = 0; i < json.length; i++){
		 			...
		 			$("#t1").append("<tr><td>" 
		 					+ json[i].did + "</td><td>"
		 					+ json[i].dname + "</td><td>"
		 					+ json[i].dnumber + "</td><td>"
		 					+ json[i].director + "</td><td>"
		 					+ json[i].dlocation + "</td><td>"
		 					+ "<button onclick='del("+json[i].did+")' class='btn btn-link'>删除</button></td></tr>");
	 			}
	 		});
	 		
	 		function del(id){
	 			//通过异步请求，AJAX删除当前的操作
	 			$.post("/Department/DeleteServlet",{"did":id},function(data){
	 				//提示删除多少号的部门
	 				alert( "部门编号为"+ data + "已被删除！");
	 				//重新刷新一下界面
